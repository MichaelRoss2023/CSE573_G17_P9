...
X = full(features);  % Converts matrix to full storage - must be nonsparse 
Y = full(label);
rng('shuffle')
...
Mdl = fitctree(XTrain,YTrain,'MaxNumSplits',1); % Train the classification tree

view(Mdl,'Mode','graph') % View the decision tree
...
recall = fraudAccurate/fraudTotal
